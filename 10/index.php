...
$stmt = $pdo->prepare("SELECT * FROM cms_table WHERE view_flg = 1 ORDER BY id DESC LIMIT :limit");
...
if($flag =  false){
	...
	while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
		...
		<dd class="news-list--note" data-href="news.php?page='.$result['id'].'"><a href="news.php?page='.$result['id'].'">'.$result['news_title'].'</a></td>';
	}
}
